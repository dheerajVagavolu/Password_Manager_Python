NOTE: Before running the application on the client/user computer, execute the following command:
                    For windows - 'pip install requirements.txt'
                    For linux - 'pip3 install requirements.txt'
      If there are any errors during installing packages, run 'sudo apt upgrade', 'sudo apt update' and then try again.
...
NOTE: The host and port arguments are not necessary as their default value is set to 127.0.0.1 and 9999. 
      To test them on same computer, 'make server' and 'make client' are sufficient
